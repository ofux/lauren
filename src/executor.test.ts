...
describe('runPlan zero-diff already-done handling', () => {
  ...
  test('routes implement/fix through the configured agent (codex)', async () => {
    // Use a Step plan so review (default: codex) is invoked too.
    vi.mocked(streamSubprocess).mockResolvedValue(0);
    vi.mocked(workingTreeDirty)
      .mockReturnValueOnce(false) // dirty-before-start
      .mockReturnValueOnce(true) // after implement
      .mockReturnValueOnce(true); // before commit
    vi.mocked(runCodexReview).mockResolvedValue({ code: 0, reviewText: 'needs work' });
    vi.mocked(gitCommit).mockReturnValue({ code: 0, stdout: '', stderr: '' });
    ...
    await runPlan({
      ...
      dryRun: false,
      targetRepos: [fakeRepo],
      agents: { ...DEFAULT_AGENTS, implement: 'codex', fix: 'codex' },
    });

    // Both implement and fix should have used `codex exec`.
    const calls = vi.mocked(streamSubprocess).mock.calls;
    expect(calls.length).toBe(2);
    expect(calls[0]?.[0]?.cmd).toEqual(['codex', 'exec', expect.stringContaining('')]);
    expect(calls[1]?.[0]?.cmd).toEqual(['codex', 'exec', expect.stringContaining('')]);
    // Review still routed through the default codex adapter.
