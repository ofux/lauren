...
/**
 * Ask the configured merger agent to resolve git merge conflicts inside the
 * parent repo whose merge conflicted. The caller commits the result with
 * `git commit --no-edit`.
 */
...
async function autoMerge(args: {
  ...
  for (const wt of worktrees) {
    ...
    const mergerAgent = getAgent(config.agents.merger);
    const resolverCode = await runConflictResolver({
      plan,
      parentRoot: wt.parentRoot,
      branch: wt.branch,
    ...
    if (resolverCode !== 0) {
      ...
      return {
        kind: 'failed',
        failure: failure(
          `${mergerAgent.name} conflict resolver exited ${resolverCode} in ${displayPath(
            wt.parentRoot,
          )}; ran \`git merge --abort\`. See ${displayPath(logPath)}.`,
        ),
      };
    }
    // The merger agent is instructed to `git add` resolved files itself, but
    // keep the defensive fallback for successful resolutions it left unstaged.
    // Stage only the paths that were unmerged before the agent ran so unrelated
    ...
    if (hasUnresolvedMergeConflicts(wt.parentRoot)) {
      ...
      return {
        kind: 'failed',
        failure: failure(
          `${mergerAgent.name} finished but ${displayPath(wt.parentRoot)} still has ` +
            `unresolved changes; ran \`git merge --abort\`. See ${displayPath(logPath)}.`,
        ),
      };
    }
